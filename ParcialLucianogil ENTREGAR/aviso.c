...
/** \brief da de alta un aviso ingresando un numero de ID, y luego completando sus campos
 *
            ...
            if(posCliente>=0)
            {
                ...
                if(!getValidInt("\nNumero Rubro","Error",&numeroRubroAux,0,1000,2))
                {
                    if(!getValidString("\nTexto aviso? ","\nEso no es un texto","El maximo es 64",textoAviso,64,2))
                    {
                        ...
                        printf("-Texto%s  -NumeroRubro%d    -idAviso%d  -idCliente%d  -idPublicacion %d",arrayAviso[i].textoDelAviso,arrayAviso[i].numeroRubro,arrayAviso[i].idAviso,arrayAviso[i].idCliente,arrayAviso[i].idAviso);
                    }
                }
            }
...
*/
/** \brief cambia el estado del campo de la estructura AVISO, de activo a pausado
 *
 * \param arrayAviso Aviso*
 * \param lenAviso int
 * \param arrayCliente Cliente*
 * \param lenCliente int
 * \param idPublicacion int
 * \return int
 *
 */
...
/** \brief cambia el estado del campo de la estructura AVISO, de pausado a activo
 *
 * \param arrayAviso Aviso*
 * \param lenAviso int
 * \param arrayCliente Cliente*
    ...
    for(i=0; i<lenAviso;i++)
    {
        if(idPublicacion==arrayAviso[i].idAviso)
        {
            for(j=0;j<lenCliente;j++)
            {
                ...
                {
                    printf(" Nombre %s  -Apellido %s -Cuit %s -IdCliente %d",arrayCliente[j].nombre,arrayCliente[j].apellido,arrayCliente[j].cuit,arrayCliente[j].idCliente);
                    getValidInt("desea cambiar estado a activa 0 es si/1 es no?","error",&auxiliar,0,1,2);
                    if(auxiliar==1)
                    {
                        strcpy(arrayAviso[i].estado,"activa");
                        retorno=0;
...
int aviso_altaForzada(Aviso* arrayAviso,int lenAviso,int idCliente,int nroRubro,char* textoAviso)
{
    ...
    if(lenAviso > 0 && arrayAviso != NULL)
    {
        ...
        {
            retorno = 0;
            arrayAviso[i].idCliente=idCliente;
            arrayAviso[i].numeroRubro=nroRubro;
            strcpy(arrayAviso[i].textoDelAviso,textoAviso);
            strcpy(arrayAviso[i].estado,"activa");
            //------------------------------
            //------------------------------
